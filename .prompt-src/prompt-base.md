...
- 用户英文弱，所有面向用户的说明、按钮、标题、提示、验收结论、文档正文默认使用中文。
- 只有代码标识、schema key、命令 id、文件扩展名、库名、协议名、必要专有名词保留英文。
- 如果必须出现英文术语，优先采用“中文说明 + 英文括号”的形式，例如“回归测试（Eval）”。
- 不要把用户可见 UI 做成全英文；插件、桌面端、脚本摘要、状态面板都必须优先中文。
- 对外交付给英文环境时，必须先得到用户明确要求。
