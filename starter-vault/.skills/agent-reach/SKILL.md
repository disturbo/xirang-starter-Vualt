...
- CLI：`agent-reach 1.4.0`，通过 `uv tool install --editable ~/.agent-reach/tools/agent-reach` 安装。
- 源码：`~/.agent-reach/tools/agent-reach`。GitHub main 直接 wheel 构建存在重复 include 问题，本机使用 editable 安装绕过。
- Exa：`mcporter config add exa https://mcp.exa.ai/mcp --scope home`，配置文件为 `~/.mcporter/mcporter.json`。
- YouTube/B站基础：`yt-dlp 2026.03.17`，并在 `~/Library/Application Support/yt-dlp/config` 配置 `--js-runtimes node`。
- 当前 `agent-reach doctor`：8/16 渠道可用。Cookie 类平台（Twitter/X、小红书、雪球等）尚未配置；不要自动导入 Cookie，除非用户明确要求。
