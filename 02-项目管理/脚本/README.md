...
| `v9-reflex-check.py` | V9 第一反射器聚合器：九源巡检、GBrain/Hermes/熵任务运行活性、去重冷却，写 Vault 外 runtime 的 `health-latest.json` | `python3 02-项目管理/脚本/v9-reflex-check.py` |
...
| `v9-iteration-ops-check.py` | V9.5 月度迭代 Ops 只读扫描，检查当前迭代入口、写入边界、agent_assignments、原型 preview 状态 | `python3 02-项目管理/脚本/v9-iteration-ops-check.py --json` |
...
- V9 运行态快照、eval latest、日志等过程文件不进入 Vault；默认写 `~/.xirang/v9-runtime/`，可用 `XIRANG_V9_RUNTIME_DIR` 覆盖。该目录避开 macOS Desktop TCC，保证 launchd 可持续读写。
