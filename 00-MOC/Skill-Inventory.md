...
- **用途**：从互联网获取行业趋势、技术框架、论文、博客、官方文档等外部信息
- **触发**：vault 内容需要联网补充；方法论研究需要行业前沿信息；竞品/工具对比需要最新数据
- **工具优先级**：WebSearch → WebFetch → curl 降级（三级降级路径）
- **关键步骤**：
  1. 确定搜集目标（关键词、候选 URL、语言偏好）
  2. 尝试原生工具（WebSearch/WebFetch）
  3. 失败时用 curl 降级（`curl -sL` + `sed` 去 HTML 标签）
  4. 内容去噪、结构化、交叉验证
  5. 按三层结构（Source/Summary/Published）入库
  6. 标注来源 URL 和时效性
- **产物路径**：`20-资料/联网搜集/` 或直接融入目标文档
- **质量线**：每条结论标注来源；单源信息标注"单源"；curl 降级前验证站点可达性；不把原始 HTML 堆进文档
- **关联教训**：[[教训库]] E-20260517-01（WebFetch 安全校验受限降级方案）

### 7. generate-diagram · 图表生成

- **用途**：为 vault 文档生成可视化图表（架构图、关系图、流程图、信息图）
- **触发**：文档需要插入图表；ASCII 框线图需要替换；新建方法论/架构文档
- **四条路径**：
  1. Mermaid handDrawn（内联，节点 <= 6）
  2. drawio XML（泳道/多角色流程）
  3. Agent 手写 SVG（全局架构/精确视觉控制）
  4. 在线 API 渲染（Graphviz/PlantUML 降级，需联网）
- **选型依据**：节点数、是否泳道、视觉精度需求、是否联网
- **关联规范**：[[Markdown文档输出规范]] §9、[[SVG架构图设计规范]]、[[流程图绘制规范]]
...
#### 10. enterprise-system-field-collection · 企业系统字段采集
...
#### 16. obsidian-knowledge-synthesis · 知识沉淀合成
...
#### 17. ai-execution-hallucination-audit · AI 幻觉审计
...
| 状态 | 含义 | 当前 Skill |
|------|------|-----------|
| [Active] | 生产可用 | start-task, ingest-source, publish-wiki, handoff-task, review-contradictions, web-research, generate-diagram, /feishu, prd-brand-audit, yijing-prd-spec, obsidian-knowledge-synthesis, flowforge |
